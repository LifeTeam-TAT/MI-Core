...
@NamedQueries(value = { @NamedQuery(name = "LifeProposalTemp.findAll", query = "SELECT m FROM LifeProposalTemp m "),
		@NamedQuery(name = "LifeProposalTemp.findByDate", query = "SELECT m FROM LifeProposalTemp m WHERE m.submittedDate BETWEEN :startDate AND :endDate"),
		@NamedQuery(name = "LifeProposalTemp.updateCompleteStatus", query = "UPDATE LifeProposalTemp m SET m.complete = :complete WHERE m.id = :id") })
@EntityListeners(IDInterceptor.class)
public class LifeProposalTemp implements Serializable, IDataModel, IProposal {
	...
	public LifeProposalTemp() {
	}
	
	

	public LifeProposalTemp(boolean complete, boolean isNonFinancialEndorse, boolean status, int periodMonth, int paymentTerm, double specialDiscount, double currencyRate,
			String id, String proposalNo, String portalId, boolean isSkipPaymentTLF, Date submittedDate, Date startDate, Date endDate, ProposalType proposalType,
			SaleChannelType saleChannelType, ProposalStatus proposalStatus, ClassificationOfHealth customerClsOfHealth, Branch branch, SalesPoints salesPoints, Customer customer,
			Organization organization, PaymentType paymentType, Agent agent, BankBranch saleBank, LifePolicy lifePolicy, List<ProposalInsuredPerson> proposalInsuredPersonList,
			List<LifeProposalAttachment> attachmentList, List<Attachment> customerMedicalCheckUpAttachmentList, List<SurveyQuestionAnswer> customerSurveyQuestionAnswerList,
			UserRecorder recorder, int version) {
		super();
	...
	public LifeProposalTemp(LifePolicy lifePolicy) {
...
//		attachment.setLifeProposalTemp(this);
...
//		this.proposalInsuredPersonList.add(proposalInsuredPerson);
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		LifeProposalTemp other = (LifeProposalTemp) obj;
		if (agent == null) {
			if (other.agent != null)
				return false;
