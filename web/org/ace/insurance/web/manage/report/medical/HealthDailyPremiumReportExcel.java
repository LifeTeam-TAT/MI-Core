...
public class HealthDailyPremiumReportExcel {
	private XSSFWorkbook wb;
	// private FormulaEvaluator evaluator;

	public HealthDailyPremiumReportExcel() {
		load();
	}

	private void load() {
		try {
			InputStream inp = this.getClass().getResourceAsStream("/report-template/medical/HealthDailyPremiumIncomeReport.xlsx");
			wb = new XSSFWorkbook(inp);
		} catch (IOException e) {
			throw new SystemException(ErrorCode.SYSTEM_ERROR, "Failed to load HealthDailyPremiumIncomeReport.xlsx template", e);
		}
	}

	public void generate(OutputStream op, List<HealthDailyPremiumReportDTO> healthDailyReportList, Date startDate, Date endDate) {
		try {
			Sheet sheet = wb.getSheet("HealthDailyPremiumReceiptReport");
			...
			for (HealthDailyPremiumReportDTO report : healthDailyReportList) {
				i = i + 1;
				index = index + 1;
